...
#simulation
for (i in 1:n_sims){
  cost[i] <- mean(rnorm(cost_num, cost_diff_mean, cost_diff_se))
  effect[i] <- mean(rnorm(effect_num, effect_diff_mean, effect_diff_se))
}
...
```{r trying with changing to standard deviation}
...
```{r trying with adding standard error as error}
...
costs <- rep(NA, num_sims)
effects <- rep(NA, num_sims)

for (i in 1:num_sims) {	
  
  #calculation for cost
  treatment_groups = rbinom(cost_num, 1, 0.5)
  cost = 0 + cost_diff_mean * treatment_groups + rnorm(1,0,cost_diff_se)
  costs[i] = mean(cost)	
  	
  #calculation for effect
  treatment_groups = rbinom(effect_num, 1, 0.5)	
  effect = 0 + effect_diff_mean * treatment_groups + rnorm(1,0,effect_diff_se)	
  effects[i] = mean(effect)
}
